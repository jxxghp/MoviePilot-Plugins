...
V3 的具体返回形状、合并方式和短路行为以对应模块合同为准。例如
[`music_lyrics_candidates`](21-register-lyrics-provider.md) 是歌词候选列表聚合接口，
插件匹配并下载文本后返回 `list[MusicLyrics]`；空列表或有效候选均不会阻断其他
插件与内置歌词源，最终由宿主择优并保存旁挂文件。

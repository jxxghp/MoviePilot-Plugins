...
class AnimeUpscale(_PluginBase):
    ...
    def api_create_jobs(self, payload: Optional[dict] = Body(default=None)) -> JSONResponse:
        ...
        try:
            ...
            planned = self._plan_jobs(selected, output_dir, bool(request.get("recursive", True)), model)
    ...
    def _plan_jobs(self, selected: Path, output_dir: Path, recursive: bool, model: str) -> list[tuple[Path, Path]]:
        ...
        return [
            (source, output_dir / (source.parent.relative_to(base) if selected.is_dir() else Path()) / f"{source.stem}.{suffix}.mkv")
            for source in files
        ]

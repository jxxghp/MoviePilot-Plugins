...
class InvitesSignin(_PluginBase):
    ...
    def _get_page_source(self, url: str, cookies=None) -> Optional[str]:
        """
        获取页面HTML源码。
        若启用浏览器仿真，使用 CloakBrowser 获取页面；否则走普通HTTP请求。
        """
        proxies = self.__get_proxies()
        proxy_server = settings.PROXY_SERVER if self._use_proxy else None

        if self._use_browser_emulation:
            logger.info(f"[浏览器仿真] 使用 CloakBrowser 请求: {url}")
            return self.__get_page_source_with_cloakbrowser(url=url, cookies=cookies, proxy=proxy_server)
    ...
    @staticmethod
    def __get_page_source_with_cloakbrowser(url: str, cookies: Optional[str] = None,
                                            proxy: Optional[dict] = None) -> Optional[str]:
        """
        使用 CloakBrowser 渲染页面，适配启用浏览器仿真的站点请求。
        """
        context = None
        page = None
        try:
            ...
            logger.error(f"CloakBrowser 请求失败: {url}, 错误: {e}")
            return None
        finally:
            if page:
                page.close()
                                        ...
                                        },
                                        {
                                         ...
                                         },
                                         {
                                             ...
                                             'content': [
                                                 ...
                                                 {'component': 'div', 'props': {'class': 'text-body-2 ml-8'}, 'text': '开启后将使用 CloakBrowser 获取页面，可绕过 Cloudflare 防护。浏览器运行环境须在 MoviePilot 中提前准备。'}
                                            ]
                                        }
                                    ]
                                }

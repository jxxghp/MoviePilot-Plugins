...
from .siteuserinfo import ISiteUserInfo
...
class ContractCheck(_PluginBase):
    ...
    def build(self, site_info: CommentedMap) -> Optional[ISiteUserInfo]:
        ...
        with requests.Session() as session:
            ...
            if render:
                # 演染模式
                html_text = self.__get_page_source_with_cloakbrowser(url=url, cookies=site_cookie,
                                                                     ua=ua, proxy=proxy_server)
            else:
                # 普通模式
                res = RequestUtils(
    ...
    @staticmethod
    def __get_page_source_with_cloakbrowser(url: str, cookies: str, ua: Optional[str] = None,
                                            proxy: Optional[dict] = None) -> Optional[str]:
        """
        使用 CloakBrowser 渲染站点首页，避免 V2 专用插件继续依赖旧浏览器 helper。
        """
        context = None
        page = None
        try:
            context = launch_context(headless=True, proxy=proxy, user_agent=ua)
            ...
            logger.error(f"CloakBrowser 渲染站点失败 {url}: {e}")
            return None
        finally:
            if page:
                page.close()

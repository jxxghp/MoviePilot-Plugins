...
def _spec(executable: str = "tool") -> EngineSpec:
    asset = ReleaseAsset(
...
def test_installer_serializes_concurrent_install(monkeypatch, tmp_path: Path):
    ...
    assert calls == 1
    assert results == [installer.managed_path, installer.managed_path]


def test_installer_uses_verified_managed_binary_without_path_lookup(monkeypatch, tmp_path: Path):
    installer = ManagedBinaryInstaller(tmp_path, _spec())
    installer.bin_dir.mkdir(parents=True)
    installer.managed_path.write_bytes(b"managed")

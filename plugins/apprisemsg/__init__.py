...
class AppriseMsg(_PluginBase):
    """通过单一后台队列向 Apprise 渠道发送 MoviePilot 通知。"""
    ...
    def init_plugin(self, config: dict = None):
        ...
        if not self._quiesce():
            logger.error("Apprise 消息发送线程尚未退出，跳过本次重新初始化")
            return
    ...
    def process_queue(self):
        ...
        while True:
            ...
            try:
                ...
                try:
                    self.apobj.notify(body=text, title=title)
    ...
    def _quiesce(self, timeout: float = None) -> bool:
        ...
        with self._lifecycle_lock:
            ...
            if thread.is_alive():
                logger.warning("Apprise 消息发送线程未在停止预算内退出")
                return False

...
<template>
  <main class="governor-shell">
    <section class="hero">
      <div>
        <p class="eyebrow">MediaGovernor 5.0.0</p>
        <h1>检查现在，找到真实问题</h1>
        <p class="lead">逐作品比较原文件、当前硬链接和 MoviePilot 应有结果。已经完成的结果会立即保存，关掉页面也不会中断。</p>
      </div>
      <div class="actions">
        <button v-if="running" class="secondary" @click="cancel">停止后续检查</button>
    ...
    <section class="status-card" aria-live="polite">
      <div class="status-line">
        <div>
          <b>{{ running ? '正在后台检查' : job.status === 'completed' ? '检查完成' : job.status === 'failed' ? '检查失败' : '等待检查' }}</b>
...
<style scoped>
.governor-shell{--ink:#12211b;--muted:#66736d;--line:#dce5df;--paper:#f7faf8;--green:#176b4d;--green2:#239267;--amber:#a86708;--red:#b33a3a;color:var(--ink);display:grid;gap:18px;padding:22px;max-width:1180px;margin:auto}.hero,.status-card,.panel{background:white;border:1px solid var(--line);border-radius:18px;box-shadow:0 10px 32px #234c3820}.hero{display:flex;justify-content:space-between;gap:24px;align-items:flex-start;padding:28px;background:linear-gradient(135deg,#f2fbf6,#fff)}.eyebrow{margin:0 0 7px;color:var(--green);font-size:12px;font-weight:800;letter-spacing:.12em;text-transform:uppercase}.hero h1,.modal h2{margin:0;font-size:30px;line-height:1.15}.lead{max-width:720px;color:var(--muted);line-height:1.65}.lead.small{font-size:14px}.actions{display:flex;align-items:flex-start;gap:9px;flex-wrap:wrap;justify-content:flex-end}button{font:inherit}.primary,.secondary,.danger{border-radius:11px;padding:10px 15px;font-weight:700;cursor:pointer}.primary{border:0;color:white;background:var(--green)}.secondary{border:1px solid var(--line);background:white;color:var(--ink)}.danger{border:0;color:white;background:var(--red)}button:disabled{opacity:.5;cursor:not-allowed}.advanced{position:relative}.advanced summary{cursor:pointer;color:var(--muted);padding:10px}.advanced[open]{min-width:210px}.advanced p{font-size:12px;color:var(--muted);max-width:220px}.status-card{padding:17px 20px}.status-line{display:flex;justify-content:space-between;align-items:center}.status-line div{display:grid;gap:3px}.status-line span{color:var(--muted);font-size:13px}.bar{height:6px;background:#edf2ef;border-radius:99px;margin-top:12px;overflow:hidden}.bar i{display:block;height:100%;background:var(--green2);transition:width .25s}.notice,.warning{font-size:13px;line-height:1.55}.notice{color:var(--green)}.warning{color:var(--amber)}.summary-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}.summary-grid article{display:grid;gap:4px;padding:17px 19px;background:var(--paper);border:1px solid var(--line);border-radius:14px}.summary-grid strong{font-size:27px}.summary-grid span{font-size:12px;color:var(--muted)}.panel{overflow:hidden}.panel header{display:flex;justify-content:space-between;align-items:center;padding:20px 22px 14px}.panel h2{margin:0;font-size:19px}.panel header>span{color:var(--muted);font-size:13px}.result-row{width:100%;display:flex;justify-content:space-between;gap:18px;text-align:left;padding:15px 22px;border:0;border-top:1px solid var(--line);background:white;cursor:pointer}.result-row:hover{background:#f6faf7}.result-row span{display:grid;gap:5px}.result-row small{color:var(--muted);line-height:1.45}.result-row i{font-style:normal;color:var(--green);white-space:nowrap}.review-panel{border-color:#ead8b6}.error-panel{border-color:#eccaca}.empty{padding:4px 22px 22px;color:var(--muted)}.backdrop{position:fixed;inset:0;background:#12211b88;display:grid;place-items:center;padding:20px;z-index:50}.modal{position:relative;width:min(920px,96vw);max-height:90vh;overflow:auto;background:white;border-radius:20px;padding:28px;box-shadow:0 24px 80px #0005}.close{position:absolute;right:18px;top:14px;border:0;background:transparent;font-size:28px;cursor:pointer}.facts{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin:20px 0}.facts div{display:grid;gap:5px;padding:12px;background:var(--paper);border-radius:10px}.facts span{font-size:12px;color:var(--muted)}.choice-area{border-top:1px solid var(--line);padding-top:18px}.candidate{width:100%;display:grid;gap:5px;text-align:left;border:1px solid var(--line);background:white;border-radius:11px;padding:12px;margin:8px 0;cursor:pointer}.candidate:hover{border-color:var(--green)}.candidate span{font-size:12px;color:var(--muted)}.preview{margin-top:22px;padding-top:18px;border-top:1px solid var(--line)}.compare-head,.compare-row{display:grid;grid-template-columns:1fr 1fr 1fr;gap:9px;padding:10px;border-bottom:1px solid var(--line)}.compare-head{background:var(--paper);font-size:12px}.compare-row span{overflow-wrap:anywhere;font-size:12px}.final-confirm{display:flex;align-items:center;justify-content:flex-end;gap:10px;margin-top:12px}@media(max-width:760px){.governor-shell{padding:12px}.hero{display:grid}.actions{justify-content:flex-start}.summary-grid{grid-template-columns:1fr 1fr}.facts{grid-template-columns:1fr}.compare-head{display:none}.compare-row{grid-template-columns:1fr}.result-row{display:grid}.modal{padding:22px 16px}}

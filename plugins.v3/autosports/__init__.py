...
from app.schemas.types import SystemConfigKey, MediaType, MessageType
from app.sdk.cache import TTLCache
from app.sdk.config import settings
from app.sdk.logging import logger
from app.sdk.media import MediaInfo, TorrentInfo, Context, MetaVideo, MetaBase, MetaInfoPath, MetaInfo
from app.sdk.network import RequestUtils, SitesHelper
from app.sdk.services import DownloaderHelper
from app.sdk.utilities import DomUtils, StringUtils, SystemUtils
...
def get_demo_competitions_config() -> str:
    desc = (
        "// 以下为自定义赛事配置示例，请参考：https://github.com/Sinterdial/MoviePilot-Plugins/blob/main/docs/Self_Defined_Competitions_Guide.md 进行配置\n"
        ...
        for result in results:
            try:
                ...
                if any (x in f'{title.lower()} {description.lower()}' for x in not_football):
                    ...
                    continue
                # 检查规则
                if self.__include and not any(re.search(r"%s" % include_word,
                                                    f"{title} {description}", re.IGNORECASE) for include_word in self.__include.split(",")):
                    logger.info(f"该种子不符合包含规则，跳过")
                    continue
                if self.__exclude and any(re.search(r"%s" % exclude_word,
                                                f"{title} {description}", re.IGNORECASE) for exclude_word in self.__exclude.split(",")):

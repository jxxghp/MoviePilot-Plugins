...
class NeoDBSync(_PluginBase):
    ...
    @staticmethod
    def __media_url(media_source, media_id, media_type) -> str:
        """按统一媒体身份生成详情链接。"""
        normalized_source = normalize_media_source(media_source)
        if normalized_source == MediaSource.Douban:
            return f"https://movie.douban.com/subject/{media_id}"
        media_path = "movie" if media_type == MediaType.MOVIE.value else "tv"
        return f"https://www.themoviedb.org/{media_path}/{media_id}"
                                            ...
                                            'content': [
                                                {
                                                    'component': 'a',
                                                    'props': {
                                                        'href': self.__media_url(media_source, media_id, mtype),
                                                        'target': '_blank'
                                                    },
                                                    'text': title
                                                }
            ...
            for result in results:
                try:
                    ...
                    if not mediainfo:
                        ...
                        continue
                    if any(
                            h.get("media_source") == mediainfo.media_source
                            and str(h.get("media_id")) == str(mediainfo.media_id)
                            for h in history
                    ):
                        logger.info(f'{mediainfo.title_year} 已处理过')
                        continue
                    # 查询缺失的媒体信息

...
- 下载目录默认复用 MoviePilot 的本地目录设置；只有需要覆盖宿主目录时才填写插件里的可选目录。
...
- 在 MoviePilot 原生“探索”中选择 `LunaTV / 苹果 CMS` 搜索并创建订阅；工作台仅用于诊断、排队和状态，不内置 m3u8 播放器。
- 目录、智能助手、TMDB、整理规则和媒体服务器无需在插件中重复配置；媒体库同步沿用 MoviePilot 已启用的服务器设置。

...
# MoviePilot V3 稳定 SDK 入口（本插件仅支持 V3，最低系统版本 3.0.0）
...
class HRBlocker(_PluginBase):
    ...
    def on_resource_selection(self, event: Event = None):
        ...
        try:
            ...
            # 按来源分类：订阅 => 订阅拦截，其他选择路径 => 资源选择
            stage = "订阅拦截" if ("subscribe" in origin.lower() or "订阅" in origin) else "资源选择"
    ...
    def hr_is_item_blocked(self, item: Any) -> bool:
        ...
        return bool(self.hr_item_block_reason(item))

    def hr_item_block_reason(self, item: Any) -> Optional[str]:
        """返回搜索结果条目的H&R拦截原因，非H&R返回 None"""
        try:
    ...
    def hr_filter_contexts(self, contexts: List[Any]) -> List[Any]:
        """从 Context 列表中剔除H&R种子（供搜索链包装器调用），并写入显示拦截记录（按标题去重）"""
        kept, removed = [], 0
        for context in contexts:
            try:
                blocked, reason = self.__is_hr_context(context)
            except Exception:
                blocked, reason = False, ""
            if blocked:
                removed += 1
                ti = getattr(context, "torrent_info", None)
                self.__add_record(title=getattr(ti, "title", "") or "",
                                  site=getattr(ti, "site_name", "") or "",
                                  reason=reason, source="搜索", stage="显示拦截", dedupe=True)
            else:
                kept.append(context)
        if removed:
    ...
    def hr_filter_event(self, event: Dict[str, Any], removed_cum: int) -> Tuple[Dict[str, Any], int]:
        """
        过滤渐进式搜索（SSE）事件中的H&R种子（供搜索链包装器调用），并写入显示拦截记录（按标题去重）
        ...
        if isinstance(items, list) and items:
            kept = []
            for it in items:
                reason = self.hr_item_block_reason(it)
                if reason:
                    ti = (it or {}).get("torrent_info") or {}
                    self.__add_record(title=ti.get("title") or "", site=ti.get("site_name") or "",
                                      reason=reason, source="搜索", stage="显示拦截", dedupe=True)
                else:
                    kept.append(it)
    ...
    def __add_record(self, title: str, site: str, reason: str, source: str, stage: str, dedupe: bool = False):
        """
        追加一条屏蔽记录（最新在前，上限 MAX_RECORDS 条，持久化到插件数据）
        :param dedupe: 为 True 时，若已存在同标题同分类的记录则跳过（防止搜索展示过滤刷屏）
        """
        try:
            if dedupe and any(r.get("title") == (title or "未知种子") and r.get("stage") == stage

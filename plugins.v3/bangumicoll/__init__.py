...
class BangumiColl(_PluginBase):
    ...
    def manage_subscriptions(self, items: Dict[int, Dict[str, Any]]):
        ...
        db_sub = {
            ...
            if str(i.media_source) == str(MediaSource.Bangumi)
            and str(i.media_id).isdigit()
        }
        # bangumi 条目
        _bgm = set(items.keys())
        # 订阅记录
        ...
        for subid, item in items.items():
            ...
            if mediainfo.type == MediaType.TV \
                ...
                if "OVA" in item.get("tags"):
                    ...
                    if tmdb_media_id and (
                            tmdb_info := self.chain.tmdb_info(tmdb_media_id, mediainfo.type, 0)
                    ):
            ...
            if exist_flag:
                ...
                continue
            elif not no_exists.get(
                    build_media_key(mediainfo.media_source, mediainfo.media_id), {}
            ).get(mediainfo.season):
                # 添加到排除
                self.update_data(key="exclude", value=subid)
            ...
            sid, msg = SubscribeChain().add(
                **self.prepare_add_args(meta, mediainfo, subid)
            )
            if not sid:
    ...
    def _season_split(self, mediainfo: MediaInfo, season: int = 1) -> Dict[int, dict]:
        ...
        if tmdb_media_id and (
                tmdb_info := self.chain.tmdb_info(tmdb_media_id, mediainfo.type, season)
        ):
            season = 1
    ...
    def prepare_add_args(
            self, meta: MetaBase, mediainfo: MediaInfo, subid: int
    ) -> Dict:
        """
        使用 Bangumi 收藏条目构造统一媒体身份订阅参数。
        """
    ...
    def get_subscribe_history(self, db: Session = None) -> set:
        ...
        try:
            result = (
                ...
                .filter(SubscribeHistory.media_source == str(MediaSource.Bangumi))
                .all()
            )
            return {
                int(i.media_id)
                for i in result
                if str(i.media_id).isdigit()
            }
        except Exception as e:
